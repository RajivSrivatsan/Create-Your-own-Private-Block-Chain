...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
          try{
            ...
            //Assigning the height of the new Block
            newBlock.height = self.chain.length;
            // Updating the time of block creation
            newBlock.time = new Date().getTime().toString().slice(0,-3);
            //assignging the previousblock hash
            if(self.chain.length > 0){
                newBlock.previousBlockHash = self.chain[self.chain.length -1].hash;
            }
            //Creating the current block hash
            ...
            return new Promise(async(resolve, reject)=>{
                ...
                if(currentTime - messageTime <=300){
                    ...
                    if(verifyMessage){
                        ...
                        const blockAdd = await self._addBlock(block);
                    resolve(blockAdd);
                    }else{
    ...
    getBlockByHash(hash) {
        let self = this;
        return new Promise((resolve, reject) => {
            const searchedblock = self.chain.filter((b)=> b.hash === hash)[0];
            if(searchedblock){
            resolve(block);
            }else{
                resolve(null);
    ...
    validateChain() {
        ...
        let errorLog = [];
        return new Promise(async (resolve, reject) => {
            // Validating each block with validate block
            for(var i=0; i<self.chain.length;i++){
                self.chain[i].validate().then((b)=>{
                    console.log("Block validated");
                }).catch((err) =>{
                    errorLog.push("Error: Block"+ i +" is not validated");
                })
            }
            const previousBlock = self.chain[i -1];
            if(previousBlock.hash !== self.previousBlock.hash){
